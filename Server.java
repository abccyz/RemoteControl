...
public class Server{
	public static void main(String args[]) {
		...
		//第二个功能：读取客户机的控制指令，并且操作本机
		OperateWindow operate=new OperateWindow();
		operate.changeServerPort(30012);//此处可以修改服务端口
...
class SendScreenImg implements Runnable{
     ...
     static final int DEFAULT_SERVER_PORT=30011;
	 int serverPort;
	 public Robot robot;
		...
		while (true) {
             try {
                 ...
                 try {
                     ...
                     zip.putNextEntry(new ZipEntry("screenshot.jpg"));
			...
			try {
				...
				if (info != null) {
					...
					if ("mouseClicked".equals(s[0].trim())) {//operateStr Model: mouseClicked,x,y,type  
						//由于加上单击事件后，鼠标按下并快速抬起 就设计到按下、抬起、单击 三个事件，将单击变为了双击 不合乎规范  所以 服务端并没有实现单击事件的监听，这里保留 不做修改
						...
						if (s.length == 4) {
							...
							String tempa1="ClientINFO:MOUSE move to "+x+","+y+" AND execute TYPE IS click "+type;
							jdbc.insert(tempa1);
						...
						if (s.length == 4) {
							...
							robot.mouseMove(x, y);
							robot.mousePress(type);
							String tempa2="ClientINFO:MOUSE move to "+x+","+y+" AND execute TYPE IS press "+type;
							jdbc.insert(tempa2);
						...
						if (s.length == 4) {
							...
							robot.mouseMove(x, y);
							robot.mouseRelease(type);
							String tempa3="ClientINFO:MOUSE move to "+x+","+y+" AND execute TYPE IS release  "+type;
							jdbc.insert(tempa3);
						...
						if (s.length == 4) {
							...
							int y = Integer.parseInt(s[2].trim());
							robot.mouseMove(x, y);
							String tempa4="ClientINFO:MOUSE move to "+x+","+y ;
							jdbc.insert(tempa4);
						...
						if (s.length == 3) {
							...
							int y = Integer.parseInt(s[2].trim());
							robot.mouseMove(x, y);
							String tempa5="ClientINFO:MOUSE move to "+x+","+y;
							jdbc.insert(tempa5);
							System.out.println("ClientINFO:MOUSE move to "+x+","+y);
						}
